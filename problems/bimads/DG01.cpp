...
/*                  DG01                */
/*----------------------------------------*/
class DG01 : public Multi_Obj_Evaluator
{

public:
  DG01(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
  ...
  {
    NOMAD::Double f1 = sin(x[0].value());
    NOMAD::Double f2 = sin(x[0].value() + 0.7);
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    p.set_HISTORY_FILE("DG01_bimads.txt");
    p.set_STATS_FILE("test_DG01.txt", "BBE OBJ");
