...
/*                  DTLZ1n2                */
/*----------------------------------------*/
class DTLZ1n2 : public Multi_Obj_Evaluator
{

public:
  DTLZ1n2(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
  ...
  {
    // number of variables and objectives
    int n = 2;
    int m = 2;
    ...
    for (int i = m - 1; i < n; ++i)
    {
      g += ((x[i] - 0.5) * (x[i] - 0.5) - cos(20 * PI * (x[i].value() - 0.5)));
    }
    g *= 100;

    // objective functions
    NOMAD::Double f1 = 0.5 * (1 + g);
    for (int i = 0; i < m - 1; ++i)
    {
      f1 *= x[i];
    }
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    p.set_HISTORY_FILE("DTLZ1n2_bimads.txt");
    p.set_STATS_FILE("test_DTLZ1n2.txt", "BBE OBJ");
