...
/*               Deb512b                  */
/*----------------------------------------*/
class Deb512b : public NOMAD::Multi_Obj_Evaluator
{

public:
  Deb512b(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
              ...
              bool &count_eval) const
  {
    ...
    if (f1 <= (beta * g))
    {
      h = 1 - pow(f1.value() / (beta * g.value()), alpha);
    }
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    p.set_HISTORY_FILE("Deb512b_bimads.txt");
    p.set_STATS_FILE("test_Deb512b.txt", "BBE OBJ");
