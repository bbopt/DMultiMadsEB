...
/*               Deb512c                  */
/*----------------------------------------*/
class Deb512c : public NOMAD::Multi_Obj_Evaluator
{

public:
  Deb512c(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
              ...
              bool &count_eval) const
  {
    ...
    NOMAD::Double f1 = 4 * x[0];
    NOMAD::Double g = 0;
    if ((x[1] >= 0) and (x[1] <= 0.4))
    {
    ...
    NOMAD::Double alpha = 0.25 + 3.75 * (g - 1);
    NOMAD::Double h = 0;
    if (f1 <= beta * g)
    {
      h = 1 - pow(f1.value() / g.value(), alpha.value());
    }
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    p.set_HISTORY_FILE("Deb512c_bimads.txt");
    p.set_STATS_FILE("test_Deb512c.txt", "BBE OBJ");
