...
/*               Deb521a                  */
/*----------------------------------------*/
class Deb521a : public NOMAD::Multi_Obj_Evaluator
{

public:
  Deb521a(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
              ...
              bool &count_eval) const
  {
    ...
    Double gmin = 1;
    Double gmax = 2;
    NOMAD::Double g = gmin + (gmax - gmin) * pow(x[1].value(), gamma);
    NOMAD::Double h = 1 - pow(f.value() / g.value(), 2);
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    p.set_HISTORY_FILE("Deb521a_bimads.txt");
    p.set_STATS_FILE("test_Deb521a.txt", "BBE OBJ");
