...
/*                  IM1                */
/*----------------------------------------*/
class IM1 : public Multi_Obj_Evaluator
{

    public:
        IM1(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
        ...
        {
            NOMAD::Double f1 = 2 * sqrt(x[0].value());
            NOMAD::Double f2 = x[0] * (1 - x[1]) + 5;
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        p.set_HISTORY_FILE("IM1_bimads.txt");
        p.set_STATS_FILE("test_IM1.txt", "BBE OBJ");
