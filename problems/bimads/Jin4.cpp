...
/*                  Jin4                */
/*----------------------------------------*/
class Jin4 : public Multi_Obj_Evaluator
{

    public:
        Jin4(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
                ...
                bool &count_eval) const
        {
            ...
            NOMAD::Double f2 = g * (1.0 - pow(f1.value() / g.value(), 0.25) 
                    - (f1 / g) * (f1 / g) * (f1 / g) * (f1 / g));
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        p.set_HISTORY_FILE("Jin4_bimads.txt");
        p.set_STATS_FILE("test_Jin4.txt", "BBE OBJ");
