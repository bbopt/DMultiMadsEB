...
/*                  LE1                */
/*----------------------------------------*/
class LE1 : public Multi_Obj_Evaluator
{

public:
    LE1(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
    ...
    {
        NOMAD::Double f1 = pow(x[0].value() * x[0].value() + x[1].value() * x[1].value(), 0.125);
        NOMAD::Double f2 = pow((x[0].value() - 0.5) * (x[0].value() - 0.5) + (x[1].value() - 0.5) * (x[1].value() - 0.5), 0.25);
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        p.set_HISTORY_FILE("LE1_bimads.txt");
        p.set_STATS_FILE("test_LE1.txt", "BBE OBJ");
