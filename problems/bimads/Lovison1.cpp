...
/*                  Lovison1                */
/*----------------------------------------*/
class Lovison1 : public Multi_Obj_Evaluator
{

public:
    Lovison1(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
    ...
    {
        NOMAD::Double f1 = -1.05 * x[0] * x[0] - 0.98 * x[1] * x[1];
        NOMAD::Double f2 = -0.99 * (x[0] - 3) * (x[0] - 3) - 1.03 * (x[1] - 2.5) * (x[1] - 2.5);
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        p.set_HISTORY_FILE("Lovison1_bimads.txt");
        p.set_STATS_FILE("test_Lovison1.txt", "BBE OBJ");
