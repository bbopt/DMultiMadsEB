...
/*                  Lovison3                */
/*----------------------------------------*/
class Lovison3 : public Multi_Obj_Evaluator
{

public:
    Lovison3(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
                ...
                bool &count_eval) const
    {
        ...
        NOMAD::Double f2 = -(x[0] - 6) * (x[0] - 6) + (x[1] + 0.3) * (x[1] + 0.3);
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        p.set_HISTORY_FILE("Lovison3_bimads.txt");
        p.set_STATS_FILE("test_Lovison3.txt", "BBE OBJ");
