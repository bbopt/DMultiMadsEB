...
/*                  Lovison4                */
/*----------------------------------------*/
class Lovison4 : public Multi_Obj_Evaluator
{

public:
    Lovison4(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
    ...
    {
        NOMAD::Double f1 = -x[0] * x[0] - x[1] * x[1] - 4 * (exp(-(x[0].value() + 2) * (x[0].value() + 2) - x[1].value() * x[1].value()) 
            + exp(-(x[0].value() - 2) * (x[0].value() - 2) - x[1].value() * x[1].value()));

        NOMAD::Double f2 = -(x[0] - 6) * (x[0] - 6) - (x[1] + 0.5) * (x[1] + 0.5);
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        p.set_HISTORY_FILE("Lovison4_bimads.txt");
        p.set_STATS_FILE("test_Lovison4.txt", "BBE OBJ");
