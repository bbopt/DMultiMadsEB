...
/*                  MOP6                */
/*----------------------------------------*/
class MOP6 : public Multi_Obj_Evaluator
{

public:
    MOP6(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
    ...
    {
        NOMAD::Double f1 = x[0];
        NOMAD::Double f2 = (1 + 10 * x[1]) * (1 - (x[0] / (1 + 10 * x[1])) * (x[0] / (1 + 10 * x[1]))
            - x[0] / (1 + 10 * x[1]) * sin(8 * PI * x[0].value()) );
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        p.set_HISTORY_FILE("MOP6_bimads.txt");
        p.set_STATS_FILE("test_MOP6.txt", "BBE OBJ");
