...
/*                  OKA1                */
/*----------------------------------------*/
class OKA1 : public Multi_Obj_Evaluator
{

public:
    OKA1(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
    ...
    {
        NOMAD::Double tmp_x1 = cos(PI/12) * x[0] - sin(PI/12) * x[1];
        NOMAD::Double tmp_x2 = sin(PI/12) * x[0] + cos(PI/12) * x[1];
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        p.set_HISTORY_FILE("OKA1_bimads.txt");
        p.set_STATS_FILE("test_OKA1.txt", "BBE OBJ");
