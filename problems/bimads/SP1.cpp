...
/*                  SP1                */
/*----------------------------------------*/
class SP1 : public Multi_Obj_Evaluator
{

public:
    SP1(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
    ...
    {
        NOMAD::Double f1 = (x[0] - 1) * (x[0] - 1) + (x[0] - x[1]) * (x[0] - x[1]);
        NOMAD::Double f2 = (x[1] - 3) * (x[1] - 3) + (x[0] - x[1]) * (x[0] - x[1]);
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        p.set_HISTORY_FILE("SP1_bimads.txt");
        p.set_STATS_FILE("test_SP1.txt", "BBE OBJ");
