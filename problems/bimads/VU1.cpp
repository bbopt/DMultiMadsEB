...
/*                  VU1                */
/*----------------------------------------*/
class VU1 : public Multi_Obj_Evaluator
{

public:
    VU1(const NOMAD::Parameters &p) : NOMAD::Multi_Obj_Evaluator(p) {}
    ...
    {
        NOMAD::Double f1 = 1 / (x[0] * x[0] + x[1] * x[1] + 1);
        NOMAD::Double f2 = x[0] * x[0] + 3 * x[1] * x[1] + 1;
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        p.set_HISTORY_FILE("VU1_bimads.txt");
        p.set_STATS_FILE("test_VU1.txt", "BBE OBJ");
